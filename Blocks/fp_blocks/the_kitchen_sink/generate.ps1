...
try {
  if (Get-Command sbt -ErrorAction SilentlyContinue) {
    sbt "runMain hardfloat.Emitthe_kitchen_sink ../../fp_blocks/the_kitchen_sink/rtl"
  } elseif (Test-Path "sbt-launch.jar") {
    java -jar sbt-launch.jar "runMain hardfloat.Emitthe_kitchen_sink ../../fp_blocks/the_kitchen_sink/rtl"
  } else {
